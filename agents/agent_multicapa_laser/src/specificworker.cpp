...
{
    cout << cam_map.size() << "CAM_MAAAAAAAAAAP"<< endl;
...
void SpecificWorker::update_laser_node(std::string laser_name, const RoboCompLaser::TLaserData &ldata)
{
    if( auto three_cam_node =G->get_node(three_frontal_camera_name); three_cam_node.has_value()){
        ...
        if( auto three_laser_node = G->get_node(laser_name); three_laser_node.has_value())
        {
            //auto laser_data = RoboCompLaser::TLaserData();
            ...
            DSR::Node laser_node = DSR::Node::create<laser_node_type>(laser_name);
            G->add_or_modify_attrib_local<laser_dists_att>(laser_node, dists);
            G->add_or_modify_attrib_local<laser_angles_att>(laser_node, angles);
            G->insert_node(laser_node);
            DSR::Edge edge = DSR::Edge::create<RT_edge_type>(three_cam_node.value().id(), laser_node.id());
