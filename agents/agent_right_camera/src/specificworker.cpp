...
void SpecificWorker::initialize(int period)
{
    ...
    try
    {
        ...
        Eigen::Translation<float, 3> right_tr(0.0, 0.0, 0.0);
        // Eigen::Translation<float, 3> right_tr(0.0963, 0., 0.0578); // dos cámaras?
        Eigen::Matrix3f right_m;
        right_m = Eigen::AngleAxisf(-0.610865, Eigen::Vector3f::UnitX())
...
void SpecificWorker::compute()
{
    ...
    if(compressed)
    {
        ...
        update_right_camera_node(virtual_frame, buffer);
    }
...
std::tuple<cv::Mat> SpecificWorker::mosaic(const Camera_Map &cam_map)
{
    ...
    const auto &[pipeL, intrL, extrL, depth_frameL, pointsL, color_frameL] = cam_map.at(serial_right);
    cv::Mat imgRight(consts.height, consts.width, CV_8UC3, (char *) color_frameL.get_data());
    return std::make_tuple(imgRight);
}
...
void SpecificWorker::show_depth_images(Camera_Map &cam_map)
{
    ...
    if(not cam_map.empty())
    {
        ...
        image_stack[serial_right].copyTo(frame_final(cv::Rect(0,0,w,h)));
...
void SpecificWorker::update_right_camera_node( const cv::Mat &v_image, const vector<uchar> compressed_data)
{
    if(auto node = G->get_node(right_camera_name); node.has_value())
    {
        ...
        qWarning() << __FUNCTION__ << "Node right_camera not found";
}

void SpecificWorker::update_laser_node(const RoboCompLaser::TLaserData &ldata)
{
    if (auto cam_right_node = G->get_node(right_camera_name); cam_right_node.has_value()) {
        if (auto laser_node = G->get_node(laser_right_name); laser_node.has_value()) {
