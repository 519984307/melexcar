...
/** \mainpage RoboComp::agent_bridge
 *
 * \section intro_sec Introduction
 *
 * The agent_bridge component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/agent_bridge --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class agent_bridge : public RoboComp::Application
{
public:
	agent_bridge (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
int ::agent_bridge::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		...
		{
			// Server adapter creation and publication
			if (not GenericMonitor::configGetString(communicator(), prefix, "FullPoseEstimationPub.Endpoints", tmp, ""))
			{
				cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy FullPoseEstimationPub";
			}
			Ice::ObjectAdapterPtr adapterFullPoseEstimationPub = communicator()->createObjectAdapterWithEndpoints("FullPoseEstimationPub", tmp);
			auto fullposeestimationpub = std::make_shared<FullPoseEstimationPubI>(worker);
			adapterFullPoseEstimationPub->add(fullposeestimationpub, Ice::stringToIdentity("fullposeestimationpub"));
			adapterFullPoseEstimationPub->activate();
			cout << "[" << PROGRAM_NAME << "]: FullPoseEstimationPub adapter created in port " << tmp << endl;
		}
		catch (const IceStorm::TopicExists&){
			cout << "[" << PROGRAM_NAME << "]: ERROR creating or activating adapter for FullPoseEstimationPub\n";
		}
		...
		{
			// Server adapter creation and publication
			if (not GenericMonitor::configGetString(communicator(), prefix, "OdometryMelex.Endpoints", tmp, ""))
			{
				cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy OdometryMelex";
			}
			Ice::ObjectAdapterPtr adapterOdometryMelex = communicator()->createObjectAdapterWithEndpoints("OdometryMelex", tmp);
			auto odometrymelex = std::make_shared<OdometryMelexI>(worker);
			adapterOdometryMelex->add(odometrymelex, Ice::stringToIdentity("odometrymelex"));
			adapterOdometryMelex->activate();
			cout << "[" << PROGRAM_NAME << "]: OdometryMelex adapter created in port " << tmp << endl;
		}
		catch (const IceStorm::TopicExists&){
			cout << "[" << PROGRAM_NAME << "]: ERROR creating or activating adapter for OdometryMelex\n";
		}
		...
		{
			// Server adapter creation and publication
			if (not GenericMonitor::configGetString(communicator(), prefix, "Radar.Endpoints", tmp, ""))
			{
				cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy Radar";
			}
			Ice::ObjectAdapterPtr adapterRadar = communicator()->createObjectAdapterWithEndpoints("Radar", tmp);
			auto radar = std::make_shared<RadarI>(worker);
			adapterRadar->add(radar, Ice::stringToIdentity("radar"));
			adapterRadar->activate();
			cout << "[" << PROGRAM_NAME << "]: Radar adapter created in port " << tmp << endl;
		}
		catch (const IceStorm::TopicExists&){
			cout << "[" << PROGRAM_NAME << "]: ERROR creating or activating adapter for Radar\n";
		}
...
int main(int argc, char* argv[])
{
	...
	::agent_bridge app(prefix, startup_check_flag);
