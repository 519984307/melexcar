...
void SpecificWorker::initialize(int period)
{
    ...
    try
    {
        cfg_back.enable_device(serial_back);
        cfg_back.enable_stream(RS2_STREAM_DEPTH, consts.width, consts.height, RS2_FORMAT_Z16, fps_depth);
        cfg_back.enable_stream(RS2_STREAM_COLOR, consts.width, consts.height, RS2_FORMAT_BGR8, fps_color);
        rs2::pipeline back_pipe;
        rs2::pipeline_profile profile_back = back_pipe.start(cfg_back);
        back_depth_intr = back_pipe.get_active_profile().get_stream(RS2_STREAM_DEPTH).as<rs2::video_stream_profile>().get_intrinsics();
        ...
        back_depth_extrinsics = back_tr;
        back_depth_extrinsics.rotate(back_m);
        std::cout << "back transform " << back_depth_extrinsics.matrix() << std::endl;
        cam_map[serial_back] = std::make_tuple(back_pipe, back_depth_intr, back_depth_extrinsics,  rs2::frame(), rs2::points(), rs2::frame());
        print_camera_params(serial_back, profile_back);
        qInfo() << __FUNCTION__ << " back-camera started";
    }
    catch(const std::exception &e)
    { std::cout << e.what()<< " Serial number:" << serial_back << std::endl; std::terminate();}
...
void SpecificWorker::compute()
{
    ...
    if(compressed)
    {
        ...
        update_back_camera_node(virtual_frame, buffer);
    }
...
std::tuple<cv::Mat> SpecificWorker::mosaic(const Camera_Map &cam_map)
{
    ...
    const auto &[pipeL, intrL, extrL, depth_frameL, pointsL, color_frameL] = cam_map.at(serial_back);
    cv::Mat imgBack(consts.height, consts.width, CV_8UC3, (char *) color_frameL.get_data());
    return std::make_tuple(imgBack);
}
...
void SpecificWorker::show_depth_images(Camera_Map &cam_map)
{
    ...
    if(not cam_map.empty())
    {
        ...
        image_stack[serial_back].copyTo(frame_final(cv::Rect(0,0,w,h)));
...
void SpecificWorker::update_back_camera_node( const cv::Mat &v_image, const vector<uchar> compressed_data)
{
    if(auto node = G->get_node(two_back_camera_name); node.has_value())
    {
...
{
    if (auto cam_back_node = G->get_node(two_back_camera_name); cam_back_node.has_value()) {
        if (auto laser_node = G->get_node(laser_back_name); laser_node.has_value()) {
