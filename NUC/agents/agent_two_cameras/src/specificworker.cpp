...
void SpecificWorker::initialize(int period)
{
    ...
    try
    {
        ...
        rs2::pipeline_profile profile_center = center_pipe.start(cfg_right);
        right_depth_intr = center_pipe.get_active_profile().get_stream(RS2_STREAM_DEPTH).as<rs2::video_stream_profile>().get_intrinsics();
        Eigen::Translation<float, 3> center_tr(0., 0.,  -0.020);
        Eigen::Matrix3f center_m;
        center_m = Eigen::AngleAxisf(0.0, Eigen::Vector3f::UnitX()) //VER SOPORTE
        ...
        cam_map[serial_right] = std::make_tuple(center_pipe, right_depth_intr, center_depth_extrinsics, rs2::frame(), rs2::points(), rs2::frame());
        print_camera_params(serial_right, profile_center);
    ...
    try
    {
        ...
        left_m = Eigen::AngleAxisf(-M_PI/3., Eigen::Vector3f::UnitX()) //VER SOPORTE
                 * Eigen::AngleAxisf(0.0, Eigen::Vector3f::UnitY())
                 * Eigen::AngleAxisf(0.0, Eigen::Vector3f::UnitZ());
...
void SpecificWorker::compute()
{
    ...
    if(compressed)
    {
        ...
        update_two_camera_node(two_back_camera_name, virtual_frame, buffer);
    }
...
//    }
    ...
    insert_laser_node();
    update_laser_node(laser_back_name,ldata_local);
}
...
RoboCompLaser::TLaserData SpecificWorker::compute_laser(const Camera_Map &cam_map_extended)
{
    ...
    for( const auto &[key, value] : cam_map_extended)
    {
        ...
        {
            if(vertices[i].z >= 0.3 and vertices[i].z <= 1.8){
...
{
   /* if (auto cam_node = G->get_node(two_back_camera_name); cam_node.has_value())
        cam_api = G->get_camera_api(cam_node.value());
    else {
        std::cout << "Controller-DSR terminate: could not find a camera node named "
                  << two_back_camera_name << std::endl;
        std::terminate();
    }*/
}

void SpecificWorker::update_two_camera_node(std::string camera_name, const cv::Mat &v_image, const vector<uchar> compressed_data)
{
    if(auto node = G->get_node(camera_name); node.has_value())
    {
        ...
        qWarning() << __FUNCTION__ << "Node two_back_camera not found";
}
...
void SpecificWorker::update_laser_node(std::string camera_name, const RoboCompLaser::TLaserData &ldata)
{
    if( auto laser_back_node = G->get_node(laser_back_name); laser_back_node.has_value())
    {
        ...
        G->add_or_modify_attrib_local<laser_dists_att>(laser_back_node.value(), dists);
        G->add_or_modify_attrib_local<laser_angles_att>(laser_back_node.value(), angles);
        G->update_node(laser_back_node.value());
    }
    else
...
{
    if (auto cam_node = G->get_node(two_back_camera_compressed_name); cam_node.has_value())
        cam_api = G->get_camera_api(cam_node.value());
    else {
        std::cout << "Controller-DSR terminate: could not find a camera node named "
                  << two_back_camera_compressed_name << std::endl;
        std::terminate();
    }
}
