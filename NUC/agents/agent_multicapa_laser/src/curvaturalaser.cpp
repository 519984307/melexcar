...
(int TamanoLaser_, int K_, int KMAX_, int KMIN_, double UmbralAdapt_,double UmbralEsquina_)
{
    ...
    ArrayIN = std::vector<DatoLASER>(TamanoLaser);
    Curvatura = std::vector<double>(2*(TamanoLaser-KMin));
    Esquinas = std::vector<double>(TamanoLaser);
}
...
int CURVATURALASER::CalculoCurvatura()
{
	int error;
...
int CURVATURALASER::ObtenerCurvaturaAdaptativo()
{
	register int i;
	int cont=0;
	int error=0;
	...
	for (i=KMin;i>0;i--)
		distkaAhora += DistEuclidea(ArrayIN[i].f,ArrayIN[i].c,ArrayIN[i-1].f,ArrayIN[i-1].c);
	for (i=KMin;i<KMin*2;i++)
		distkdAhora += DistEuclidea(ArrayIN[i].f,ArrayIN[i].c,ArrayIN[i+1].f,ArrayIN[i+1].c);
    double dist = 0;
	do
    {
		kdAhora++;
		distkdAhora += DistEuclidea(ArrayIN[KMin+kdAhora-1].f,ArrayIN[KMin+kdAhora-1].c,ArrayIN[KMin+kdAhora].f,ArrayIN[KMin+kdAhora].c);
        dist = DistEuclidea(ArrayIN[KMin].f,ArrayIN[KMin].c, ArrayIN[KMin+kdAhora].f,ArrayIN[KMin+kdAhora].c);
    }
	while (dist + UmbralAdapt > distkdAhora && kdAhora < KMax);

    kdAhora--;
	distkdAhora -= DistEuclidea(ArrayIN[KMin+kdAhora].f,ArrayIN[KMin+kdAhora].c, ArrayIN[KMin+kdAhora+1].f,ArrayIN[KMin+kdAhora+1].c);
	...
	for (i=KMin;i<TamanoLaser-KMin;i++)
    {
		...
		if (i+kdAhora+1<TamanoLaser)
        {
			...
			if (DistEuclidea(ArrayIN[i+1].f,ArrayIN[i+1].c,ArrayIN[i+kdAhora+1].f,ArrayIN[i+kdAhora+1].c)+UmbralAdapt<distkdAhora && kdAhora>KMin)
            {
			...
			if (DistEuclidea(ArrayIN[i+1].f,ArrayIN[i+1].c,ArrayIN[i+kdAhora+1].f,ArrayIN[i+kdAhora+1].c)+UmbralAdapt>distkdAhora && kdAhora<KMax && i+kdAhora+2<TamanoLaser)
            {
				kdAhora++;
				distkdAhora=distkdAhora+DistEuclidea(ArrayIN[i+kdAhora].f,ArrayIN[i+kdAhora].c,
													 ArrayIN[i+kdAhora+1].f,ArrayIN[i+kdAhora+1].c);
		...
		distkaAhora=distkaAhora+DistEuclidea(ArrayIN[i].f,ArrayIN[i].c,ArrayIN[i+1].f,ArrayIN[i+1].c)-DistEuclidea(ArrayIN[i-kaAhora].f,ArrayIN[i-kaAhora].c,ArrayIN[i-kaAhora+1].f,ArrayIN[i-kaAhora+1].c);
        ...
        if (DistEuclidea(ArrayIN[i+1].f,ArrayIN[i+1].c,ArrayIN[i-kaAhora+1].f,ArrayIN[i-kaAhora+1].c)+UmbralAdapt<distkaAhora && kaAhora>KMin)
        {
		...
		if (DistEuclidea(ArrayIN[i+1].f,ArrayIN[i+1].c,ArrayIN[i-kaAhora+1].f,ArrayIN[i-kaAhora+1].c)+UmbralAdapt>distkaAhora && kaAhora<KMax)
        {
...
int CURVATURALASER::DetectarEsquinas()
{
	int i; 
	int ANGLE;
	int SIGNO;
...
void CURVATURALASER::DataInPixels(std::vector<float> x_coords, std::vector<float> y_coords, int inicio)
{
	for (int i= 0; i < TamanoLaser; i++)
    {
		ArrayIN[i].f=x_coords[i+inicio];
		ArrayIN[i].c=y_coords[i+inicio];
    }
}
