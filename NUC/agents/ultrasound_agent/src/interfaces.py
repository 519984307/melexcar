...
class SensorParamsList(list):
    def __init__(self, iterable=list()):
        super(SensorParamsList, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompUltrasound.SensorParams)
        super(SensorParamsList, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompUltrasound.SensorParams)
        super(SensorParamsList, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompUltrasound.SensorParams)
        super(SensorParamsList, self).insert(index, item)

setattr(RoboCompUltrasound, "SensorParamsList", SensorParamsList)
class SensorsState(list):
    def __init__(self, iterable=list()):
        super(SensorsState, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, int)
        super(SensorsState, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, int)
        super(SensorsState, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, int)
        super(SensorsState, self).insert(index, item)

setattr(RoboCompUltrasound, "SensorsState", SensorsState)
class SonarPoseList(list):
    def __init__(self, iterable=list()):
        super(SonarPoseList, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompUltrasound.SonarPose)
        super(SonarPoseList, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompUltrasound.SonarPose)
        super(SonarPoseList, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompUltrasound.SonarPose)
        super(SonarPoseList, self).insert(index, item)

setattr(RoboCompUltrasound, "SonarPoseList", SonarPoseList)
...
class Implements:
    def __init__(self, ice_connector, default_handler):
        self.ice_connector = ice_connector
        self.ultrasound = self.create_adapter("Ultrasound", ultrasoundI.UltrasoundI(default_handler))
