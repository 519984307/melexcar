...
private:
    ...
    struct CONSTANTS
    {
        ...
        int MAX_LASER_BINS = 87;  // 2 for each degrees
        float TOTAL_HOR_ANGLE = 87*M_PI/180;  // D455 opens 87 x 58 degrees
    ...
    // camera
    std::string serial_center, serial_left, serial_right, serial_back;
    ...
    rs2::config cfg_center, cfg_left, cfg_right, cfg_back;
    rs2_intrinsics center_cam_intr, left_cam_intr, right_cam_intr, center_depth_intr, back_depth_intr,left_depth_intr, right_depth_intr;
    rs2::context ctx;
...
//    void insert_camera_node();
    void update_back_camera_node(const cv::Mat &v_image, const vector<uchar> compressed_data);
//    void insert_camera_node_compressed();
    ...
    void update_laser_node(const RoboCompLaser::TLaserData &ldata);
};
