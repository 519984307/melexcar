...
void SpecificWorker::compute() {
    ...
    if(auto node_path = G->get_node(current_path_name); node_path.has_value())
    {
        ...
        auto speeds_sim = std::make_tuple(0.0, 0.0, 0.0);
        ...
        if(is_cyclic.load())
        {
            ...
            std::cout << "Speeds antes"<<std::get<0>(speeds) << std::endl;
//            speeds_sim = sim_factor(speeds);
            std::cout << "Speeds despues"<< std::get<0>(speeds_sim) << std::endl;
...
//    rotVel = (Vi-Vd)*consts.rotation_gain/consts.robot_width;
    ...
    auto ang_ruedas = (atan2(DeltaX, DeltaY)*180/M_PI);

    ang_ruedas = std::clamp(ang_ruedas, -35.0, 35.0);
    cout << ang_ruedas << endl;

    auto OldRange = 70;
    auto NewRange = 2;
    auto NewValue = (((ang_ruedas - (-35)) * NewRange) / OldRange) + (-1);
    cout << "Angulo_rueda: " <<ang_ruedas << "Valor de giro:"<<NewValue<< endl;


//    rotVel=modify_rotation_value(NewValue);
...
//    advVel = consts.max_adv_speed * (Vd + Vi) ;
    ...
    advVel = sim_factor(advVel);
//    std::cout<<"ADVVEL*exponential"<< advVel*exponentialFunction(rotVel, consts.advance_gaussian_cut_x, consts.advance_gaussian_cut_y, 0)<<endl;
    advVel = std::min(advVel * exponentialFunction(rotVel, consts.advance_gaussian_cut_x, consts.advance_gaussian_cut_y, 0.8), euc_dist_to_target);
//    std::cout<<"ADVVEL_sinCLAMP"<< advVel<<endl;
//    advVel = std::clamp(float(advVel), float(0.53), float(0.68));
    std::cout<<"ADVVEL"<< advVel<<endl;
//    std::cout<<"EXPONENCIAL"<< exponentialFunction(rotVel, consts.advance_gaussian_cut_x, consts.advance_gaussian_cut_y, 0)<<endl;
//    std::cout << " DeltaY " << DeltaY << " DeltaX: " << DeltaX << std::endl;
//    std::cout << " Vd " << Vd << " Vi: " << Vi << std::endl;
//    std::cout << " Adv: " << advVel << " rotVel: " << rotVel << std::endl;

    if ((robot_nose-vtp).norm()< consts.final_distance_to_target*2.5) { //real robot
...
float SpecificWorker::sim_factor(float vel)
{
...
//                    std::cout<<v.value().index()<<endl;
                    ...
                    k_percentage = exponentialFunction2(collision_percentage, 0.5, 0.53/0.7, 0);
                    ...
                    k_time = 1- exponentialFunction2(min_time ,5,0.3,0);
                }
...
//                    std::cout<<v.value().index()<<endl;
                ...
                k_brake = exponentialFunction2(brake_percentage, 0.6, 0.53/0.7, 0);
            }
        }
    }
...
float SpecificWorker::exponentialFunction2(float value, float xValue, float yValue, float min)
{
    if (yValue <= 0)
        return 1.f;
    float landa = -pow(xValue,2) / log(yValue);
    float res = exp(-value / landa);
    return std::max(res, min);
}
...
float SpecificWorker::modify_rotation_value(float rot){
    float new_rot;
    if (abs(rot)<0.3){
        new_rot=1.65*rot;
    }
