...
bool SpecificWorker::setParams(RoboCompCommonBehavior::ParameterList params)
{
    ...
    origin_x = stoi(params["x"].value);
    origin_y = stoi(params["y"].value);
    origin_z = stoi(params["z"].value);
    origin_rx = stoi(params["rx"].value);
    origin_ry = stoi(params["ry"].value);
    origin_rz = stoi(params["rz"].value);
	...
	else
	{
		...
		if(robot_real)
		{
            ...
            try {
                ...
                // float x = -3085;
                //float y = -14043;
                float x = origin_x; //3118; //9300; //10143;//14963-13*340-400;//-3171;
                float y = origin_y; //17099;//5525; //18225-13600+900;//-6680;
                float z = origin_z;
                float rx = origin_rx;
                float ry = origin_ry;
                float rz = origin_rz ;//-90;
...
void SpecificWorker::compute()
{
    ...
    if (robot_real)
    {
        //Llamada a metodo para guardar la imagen de la cámara usb del robot
        //update_camera_simple_usb();
        //Llamada a metodo para guardar la imagen de las 3cámaras del robot
//        update_three_camera();
//        update_back_camera();
//        update_right_camera();
//        update_left_camera();
        // get laser data from robot and call update_laser
//        auto laser_front = read_laser_front_robot();
//        auto laser_back = read_laser_back_robot();
//        auto laser_left = read_laser_left_robot();
//        auto laser_right = read_laser_right_robot();
//        update_laser(laser_front, laser_front_name);
//        update_laser(laser_back, laser_back_name);
//        update_laser(laser_left, laser_left_name);
//        update_laser(laser_right, laser_right_name);
...
std::vector<SpecificWorker::LaserPoint> SpecificWorker::read_laser_front_robot()
{
    ...
    try
    {
        auto laser = laser3_proxy->getLaserData();
...
std::vector<SpecificWorker::LaserPoint> SpecificWorker::read_laser_left_robot()
{
    std::vector<LaserPoint> laser_data;
    ...
    {
        cout<< "HOLA"<< endl;
        auto laser = laser1_proxy->getLaserData();
        cout <<"ADIOS"<<endl;
        std::transform(laser.begin(), laser.end(), std::back_inserter(laser_data), [](const auto &l) {return LaserPoint{l.dist, l.angle}; });
    }catch (const Ice::Exception &e){ std::cout << e.what() << " No laser_pioneer_data left" << std::endl; return {};}

    return laser_data;
}
std::vector<SpecificWorker::LaserPoint> SpecificWorker::read_laser_right_robot()
{
    std::vector<LaserPoint> laser_data;

    try
    {
        auto laser = laser2_proxy->getLaserData();
        std::transform(laser.begin(), laser.end(), std::back_inserter(laser_data), [](const auto &l) {return LaserPoint{l.dist, l.angle}; });
    }catch (const Ice::Exception &e){ std::cout << e.what() << " No laser_pioneer_data right" << std::endl; return {};}

    return laser_data;
}
std::vector<SpecificWorker::LaserPoint> SpecificWorker::read_laser_back_robot()
{
    std::vector<LaserPoint> laser_data;
    ...
    try
    {
        ...
        std::transform(laser.begin(), laser.end(), std::back_inserter(laser_data), [](const auto &l) {return LaserPoint{l.dist, l.angle}; });
    }catch (const Ice::Exception &e){ std::cout << e.what() << " No laser_pioneer_data back" << std::endl; return {};}
...
void SpecificWorker::update_odometry()
{
    if( auto node = G->get_node(odometry_name); node.has_value())
    {
        auto odometry_values = odometrymelex_proxy->getOdometryState();

        // update odometry in DSR
        G->add_or_modify_attrib_local<odometry_vel_att>(node.value(), (float)odometry_values.vel);
        G->add_or_modify_attrib_local<odometry_steer_att>(node.value(), (float)odometry_values.steer);
        G->update_node(node.value());
...
void SpecificWorker::update_camera_simple_usb() {

    auto image = this->camerasimple_proxy->getImage();

    cv::Mat frame_uncompressed = cv::imdecode(image.image, -1);
    //cv::cvtColor(frame_uncompressed, frame_uncompressed, cv::COLOR_BGR2RGB);
    //cv::imshow("RGB image", frame_uncompressed);

    if(auto node = G->get_node(wide_angle_camera_name); node.has_value())
    {
        ...
        G->add_or_modify_attrib_local<cam_rgb_width_att>(node.value(), frame_uncompressed.cols);
        G->add_or_modify_attrib_local<cam_rgb_height_att>(node.value(), frame_uncompressed.rows);
        G->add_or_modify_attrib_local<cam_rgb_depth_att>(node.value(), frame_uncompressed.depth());
...
void SpecificWorker::update_three_camera()
{
    RoboCompCameraRGBDSimple::TImage rgb;
        ...
        rgb = camerargbdsimple3_proxy->getImage(three_frontal_camera_compressed_name);
...
void SpecificWorker::update_left_camera()
{
    RoboCompCameraRGBDSimple::TImage rgb;

    rgb = camerargbdsimple1_proxy->getImage(left_camera_name);
    cv::Mat frame_uncompressed = cv::imdecode(rgb.image, -1);
    //cv::cvtColor(frame_uncompressed, frame_uncompressed, cv::COLOR_BGR2RGB);
    //cv::imshow("RGB image", frame_uncompressed);

    if( auto left_camera = G->get_node(left_camera_name); left_camera.has_value())
    {
        std::vector<uint8_t> rgb; rgb.assign(frame_uncompressed.data, frame_uncompressed.data + frame_uncompressed.total()*frame_uncompressed.channels());
        G->add_or_modify_attrib_local<cam_rgb_att>(left_camera.value(), rgb);
        G->add_or_modify_attrib_local<cam_rgb_width_att>(left_camera.value(), frame_uncompressed.cols);
        G->add_or_modify_attrib_local<cam_rgb_height_att>(left_camera.value(), frame_uncompressed.rows);
        G->add_or_modify_attrib_local<cam_rgb_depth_att>(left_camera.value(), frame_uncompressed.depth());
        G->add_or_modify_attrib_local<cam_rgb_cameraID_att>(left_camera.value(),1);
        G->add_or_modify_attrib_local<cam_rgb_alivetime_att>(left_camera.value(), (int)std::chrono::time_point_cast<std::chrono::milliseconds>(MyClock::now()).time_since_epoch().count());
        G->update_node(left_camera.value());
    }
    else
        qWarning() << __FUNCTION__ << "Node three_frontal_camera not found";
}
...
void SpecificWorker::update_right_camera()
{
    RoboCompCameraRGBDSimple::TImage rgb;

    rgb = camerargbdsimple2_proxy->getImage(right_camera_name);
    cv::Mat frame_uncompressed = cv::imdecode(rgb.image, -1);
    //cv::cvtColor(frame_uncompressed, frame_uncompressed, cv::COLOR_BGR2RGB);
    //cv::imshow("RGB image", frame_uncompressed);

    if( auto right_camera = G->get_node(right_camera_name); right_camera.has_value())
    {
        std::vector<uint8_t> rgb; rgb.assign(frame_uncompressed.data, frame_uncompressed.data + frame_uncompressed.total()*frame_uncompressed.channels());
        G->add_or_modify_attrib_local<cam_rgb_att>(right_camera.value(), rgb);
        G->add_or_modify_attrib_local<cam_rgb_width_att>(right_camera.value(), frame_uncompressed.cols);
        G->add_or_modify_attrib_local<cam_rgb_height_att>(right_camera.value(), frame_uncompressed.rows);
        G->add_or_modify_attrib_local<cam_rgb_depth_att>(right_camera.value(), frame_uncompressed.depth());
        G->add_or_modify_attrib_local<cam_rgb_cameraID_att>(right_camera.value(),1);
        G->add_or_modify_attrib_local<cam_rgb_alivetime_att>(right_camera.value(), (int)std::chrono::time_point_cast<std::chrono::milliseconds>(MyClock::now()).time_since_epoch().count());
        G->update_node(right_camera.value());
    }
    else
        qWarning() << __FUNCTION__ << "Node three_frontal_camera not found";
}
...
void SpecificWorker::update_back_camera()
{
    RoboCompCameraRGBDSimple::TImage rgb;

    rgb = camerargbdsimple_proxy->getImage(two_back_camera_name);
    cv::Mat frame_uncompressed = cv::imdecode(rgb.image, -1);
    //cv::cvtColor(frame_uncompressed, frame_uncompressed, cv::COLOR_BGR2RGB);
    //cv::imshow("RGB image", frame_uncompressed);

    if( auto back_camera = G->get_node(two_back_camera_name); back_camera.has_value())
    {
        std::vector<uint8_t> rgb; rgb.assign(frame_uncompressed.data, frame_uncompressed.data + frame_uncompressed.total()*frame_uncompressed.channels());
        G->add_or_modify_attrib_local<cam_rgb_att>(back_camera.value(), rgb);
        G->add_or_modify_attrib_local<cam_rgb_width_att>(back_camera.value(), frame_uncompressed.cols);
        G->add_or_modify_attrib_local<cam_rgb_height_att>(back_camera.value(), frame_uncompressed.rows);
        G->add_or_modify_attrib_local<cam_rgb_depth_att>(back_camera.value(), frame_uncompressed.depth());
        G->add_or_modify_attrib_local<cam_rgb_cameraID_att>(back_camera.value(),1);
        G->add_or_modify_attrib_local<cam_rgb_alivetime_att>(back_camera.value(), (int)std::chrono::time_point_cast<std::chrono::milliseconds>(MyClock::now()).time_since_epoch().count());
        G->update_node(back_camera.value());
    }
    else
        qWarning() << __FUNCTION__ << "Node back_camera not found";
}


void SpecificWorker::read_battery()
{
    RoboCompBatteryStatus::TBattery battery_level;

    try
    {
        battery_level = batterystatus_proxy->getBatteryState();
        if( auto battery = G->get_node(battery_name); battery.has_value())
        {
            ...
            G->add_or_modify_attrib_local<battery_A_att>(battery.value(), (float)battery_level.current);
            G->add_or_modify_attrib_local<battery_CE_att>(battery.value(), (float)battery_level.consumptionperhour);
            G->add_or_modify_attrib_local<battery_P_att>(battery.value(), (float)battery_level.power);
            G->add_or_modify_attrib_local<battery_TTG_att>(battery.value(), (float)battery_level.timetogo);
            G->add_or_modify_attrib_local<battery_V_att>(battery.value(), (float)battery_level.voltage);
            G->update_node(battery.value());
        }
    }
    catch(const Ice::Exception &e) { std::cout << "No battery node found" << std::endl;}
}
/*
...
void SpecificWorker::update_gps()
{
    try
    {
        ...
        if( auto gps = G->get_node(gps_name); gps.has_value())
        {
            ...
            G->add_or_modify_attrib_local<gps_azimut_att>(gps.value(), (float)gps_state.azimut);
            G->add_or_modify_attrib_local<gps_rot_att>(gps.value(), (float)gps_state.rot);
            G->add_or_modify_attrib_local<gps_UTMx_att>(gps.value(), (float)gps_state.UTMx);
            G->add_or_modify_attrib_local<gps_UTMy_att>(gps.value(), (float)gps_state.UTMy);
...
void SpecificWorker::modify_attrs_slot(std::uint64_t id, const std::vector<std::string>& att_names)
{
    if(id == 200)
    {
        qInfo() << __FUNCTION__ << att_names.size() << id;
        ...
        {
            qInfo() << __FUNCTION__ << " ENTROOOO" << id;
            if (auto robot = G->get_node(robot_name); robot.has_value())
            {
                auto ref_adv_speed = G->get_attrib_by_name<robot_ref_adv_speed_att>(robot.value());
                auto ref_rot_speed = G->get_attrib_by_name<robot_ref_rot_speed_att>(robot.value());
...
/**************************************/
// From the RoboCompCameraRGBDSimple you can call this methods:
// this->camerargbdsimple1_proxy->getAll(...)
// this->camerargbdsimple1_proxy->getDepth(...)
// this->camerargbdsimple1_proxy->getImage(...)
...
/**************************************/
// From the RoboCompCameraRGBDSimple you can call this methods:
// this->camerargbdsimple2_proxy->getAll(...)
// this->camerargbdsimple2_proxy->getDepth(...)
// this->camerargbdsimple2_proxy->getImage(...)
...
/**************************************/
// From the RoboCompCameraRGBDSimple you can call this methods:
// this->camerargbdsimple3_proxy->getAll(...)
// this->camerargbdsimple3_proxy->getDepth(...)
// this->camerargbdsimple3_proxy->getImage(...)
...
/**************************************/
// From the RoboCompLaser you can call this methods:
// this->laser3_proxy->getLaserAndBStateData(...)
// this->laser3_proxy->getLaserConfData(...)
// this->laser3_proxy->getLaserData(...)
...
/**************************************/
// From the RoboCompOdometryMelex you can call this methods:
// this->odometrymelex_proxy->getOdometryState(...)
