...
/** \mainpage RoboComp::melex_dsr
 *
 * \section intro_sec Introduction
 *
 * The melex_dsr component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/melex_dsr --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class melex_dsr : public RoboComp::Application
{
public:
	melex_dsr (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
int ::melex_dsr::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	RoboCompCameraRGBDSimple::CameraRGBDSimplePrxPtr camerargbdsimple2_proxy;
	RoboCompCameraRGBDSimple::CameraRGBDSimplePrxPtr camerargbdsimple3_proxy;
	...
	RoboCompLaser::LaserPrxPtr laser3_proxy;
	RoboCompOdometryMelex::OdometryMelexPrxPtr odometrymelex_proxy;
	...
	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "CameraRGBDSimple2Proxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy CameraRGBDSimpleProxy\n";
		}
		camerargbdsimple2_proxy = Ice::uncheckedCast<RoboCompCameraRGBDSimple::CameraRGBDSimplePrx>( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception creating proxy CameraRGBDSimple2: " << ex;
		return EXIT_FAILURE;
	}
	rInfo("CameraRGBDSimpleProxy2 initialized Ok!");


	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "CameraRGBDSimple3Proxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy CameraRGBDSimpleProxy\n";
		}
		camerargbdsimple3_proxy = Ice::uncheckedCast<RoboCompCameraRGBDSimple::CameraRGBDSimplePrx>( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception creating proxy CameraRGBDSimple3: " << ex;
		return EXIT_FAILURE;
	}
	rInfo("CameraRGBDSimpleProxy3 initialized Ok!");
	...
	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "Laser3Proxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy LaserProxy\n";
		}
		laser3_proxy = Ice::uncheckedCast<RoboCompLaser::LaserPrx>( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception creating proxy Laser3: " << ex;
		return EXIT_FAILURE;
	}
	rInfo("LaserProxy3 initialized Ok!");


	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "OdometryMelexProxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy OdometryMelexProxy\n";
		}
		odometrymelex_proxy = Ice::uncheckedCast<RoboCompOdometryMelex::OdometryMelexPrx>( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception creating proxy OdometryMelex: " << ex;
		return EXIT_FAILURE;
	}
	rInfo("OdometryMelexProxy initialized Ok!");
	...
	tprx = std::make_tuple(batterystatus_proxy,camerargbdsimple_proxy,camerargbdsimple1_proxy,camerargbdsimple2_proxy,camerargbdsimple3_proxy,camerasimple_proxy,differentialrobot_proxy,fullposeestimation_proxy,gpsublox_proxy,laser_proxy,laser1_proxy,laser2_proxy,laser3_proxy,odometrymelex_proxy,rssistatus_proxy,radar_proxy);
...
int main(int argc, char* argv[])
{
	...
	::melex_dsr app(prefix, startup_check_flag);
